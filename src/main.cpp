...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Creepercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Creepercoin Signed Message:\n";
...
{
    // Creepercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Creepercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Creepercoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Creepercoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Creepercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Creepercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Creepercoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0x1b, 0xc2, 0xb2, 0xda }; // Creepercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CreepercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CreepercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CreepercoinMiner(CWallet *pwallet)
{
    printf("CreepercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("creepercoin-miner");
    ...
    try { loop {
        ...
        printf("Running CreepercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CreepercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CreepercoinMiner, pwallet));
}
